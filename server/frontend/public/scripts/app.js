...
function setupHtmlEvents() {
    ...
    window.addEventListener("fullscreenchange", resizePlayerStyle, true);
...
function setupWebRtcPlayer(htmlElement, config) {
    ...
    //add by hzy, for auto play
    if (auto_connect_without_interactive) {
        webRtcPlayerObj.video.muted = true;
        webRtcPlayerObj.video.style.objectFit = "fill";
    }
...
function emitDescriptor(messageType, descriptor, largeSizeBuffer = true) {
    ...
    byteIdx++;
    if (largeSizeBuffer) {
        data.setUint32(byteIdx, descriptorAsString.length, true);
        byteIdx += 4;
    } else {
...
function emitUIInteraction(descriptor, largeSizeBuffer = true) {
    emitDescriptor(MessageType.UIInteraction, descriptor, largeSizeBuffer);
}
    ...
    if (playerElement && videoElement.length > 0) {
        ...
        if (playerAspectRatio > videoAspectRatio) {
            ...
            normalizeAndQuantizeUnsigned = (x, y) => {
                ...
                if (normalizedX >= 1) normalizedX = 0.999999;
                if (normalizedY < 0) normalizedY = 0;
                if (normalizedY >= 1) normalizedY = 0.999999;
...
function registerKeyboardEvents() {
    ...
    //add by hzy
    //����alt + tab������û�д���keyup��������һֱ��ס��alt�����������̲�����Ч
    window.addEventListener('blur', function (e) {
        for (var i in waitKeyUp) {
            console.log("I find lost keyup: " + waitKeyUp[i].key);
            sendInputData(new Uint8Array([MessageType.KeyUp, getKeyCode(waitKeyUp[i])]).buffer);
        }
        waitKeyUp.length = 0;
    })
